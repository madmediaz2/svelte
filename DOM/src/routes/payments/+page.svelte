<script>
  ...
  async function createPaymentIntent() {
    ...
    const response = await fetch('/create-payment-intent/ideal/payment-intent', {
      method: 'POST',
      headers: {
  ...
  async function submit() {
    ...
    const result = await stripe.confirmIdealPayment(clientSecret, {
      ...
      return_url: `${window.location.origin}/create-payment-intent/ideal/return`
    })
    ...
    if (result.error) {
      ...
      goto('/create-payment-intent/ideal/thanks')
    }
  }
</script>
